...
from dtos.id_produto_dto import ExcluirProdutoDTO, IdProdutoDTO 
...
async def obter_produto_por_id(id_produto: int = Path(..., title="Id do Produto", ge=1)):
...
async def alterar_produto(inputDto:AlterarProdutoDTO):
    ...
    pb = ProblemDetailsDto("int", f" O produto com id <b> {inputDto.id}</b> não foi encontrado.", "value_not_found", ["body", "id"]) 
    return JSONResponse(pb.to_dict(),status_code = 404)
